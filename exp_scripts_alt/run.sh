...
if [ $target == "local" ] ; then
    ...
    echo "pymarl_path: ${ICNN_PATH}" >> $sfilepath
    ...
    for i in $(seq 1 $reps); do
        ...
        NV_GPU=${gpu_id} ${ICNN_PATH}/docker.sh ${HASH} python3 src/main.py ${cmd_line} --exp_name ${name}__repeat${i} &
        echo "repeat: ${i}"
